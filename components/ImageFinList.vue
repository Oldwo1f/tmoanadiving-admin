<template>
<div>
	<b-modal id="modalRemoveImageFin" header-bg-variant="danger" @ok="modalConfirmedImageFin"
	...
	<div class="ImageListContainer">
		<div v-for="image in images" :key="image.id" class="item">
			...
			<b-btn  class=" myimageBTn btn-danger btn-sm" v-b-modal.modalRemoveImageFin @click="setToRemoveIdImageFin(image.id)"><span class="material-icons">
				delete
			</span></b-btn>
		</div>
	</div>
...
const setToRemoveIdImageFin = (val)=> {
	console.log('SETIDTOREMOVE FIN==>',val);
  idToRemove = val
}
const modalConfirmedImageFin  = async function(val){
