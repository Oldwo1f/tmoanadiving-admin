<template>
<div>
	<b-modal id="modalRemoveImageGagnant" header-bg-variant="danger" @ok="modalConfirmedImageGagnant"
	...
	<div class="ImageListContainer">
		<div v-for="image in images" :key="image.id" class="item">
			...
			<b-btn  class=" myimageBTn btn-danger btn-sm" v-b-modal.modalRemoveImageGagnant @click="setToRemoveIdImageGagnant(image.id)"><span class="material-icons">
				delete
			</span></b-btn>
		</div>
	</div>
...
const setToRemoveIdImageGagnant= (val)=> {
	console.log('SETIDTOREMOVE Gagnant==>',val);
  idToRemove = val
}
const modalConfirmedImageGagnant  = async function(val){
