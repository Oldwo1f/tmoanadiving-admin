<template>
	...
	<div class="card">
			...
			<h4 class="card-title">Déinscriptions</h4>
			<p v-if="model">Il y a <strong>{{model.length}}</strong> deinscrits à ce jeu.</p>
			<vue-json-to-csv
              :json-data="model"
              :labels="{
              ...
              :csv-title="'Liste des deinscriptions'"
              >
...
let datas=computed({
	...
	console.log(datas);
	let model=ref();
	watchEffect(() => {console.log('watchEffect!!!!!!!'); model.value= state['jeux']['currentJeu']['deinscrits'];console.log('model.value',model.value);})
